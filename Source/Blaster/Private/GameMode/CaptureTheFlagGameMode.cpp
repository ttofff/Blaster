// Fill out your copyright notice in the Description page of Project Settings.


#include "GameMode/CaptureTheFlagGameMode.h"
#include "CaptureTheFlag/FlagZone.h"
#include "GameState/BlasterGameState.h"
#include "Weapon/Flag.h"

void ACaptureTheFlagGameMode::PlayerEliminated(ABlasterCharacter* EliminatedPlayer, APlayerController* VictimController,
	APlayerController* AttackerController)
{
	//Super::PlayerEliminated(EliminatedPlayer, VictimController, AttackerController);
	ABlasterGameMode::PlayerEliminated(EliminatedPlayer, VictimController, AttackerController);//淘汰玩家
	
}

void ACaptureTheFlagGameMode::FlagCaptured(AFlag* Flag, AFlagZone* Zone)
{
	bool bValidCapture = Flag->GetTeam() != Zone->Team;
	ABlasterGameState* BGameState = Cast<ABlasterGameState>(GameState);
	if(BGameState)
	{
		if (Zone->Team == ETeam::ET_BlueTeam)
		{
			BGameState->BlueTeamScores();
		}
		if (Zone->Team == ETeam::ET_RedTeam)
		{
			BGameState->RedTeamScores();
		}
	}
}
