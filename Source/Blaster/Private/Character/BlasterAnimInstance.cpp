...
void UBlasterAnimInstance::NativeUpdateAnimation(float DeltaTimeX)
{
	...
	bUseAimOffsets = BlasterCharacter->GetCombatState() == ECombatState::ECS_Unoccupied && !BlasterCharacter->GetDisableGameplay() && !BlasterCharacter->bIsPose;
	bTransformRightHand = BlasterCharacter->GetCombatState() == ECombatState::ECS_Unoccupied && !BlasterCharacter->GetDisableGameplay() && !BlasterCharacter->bIsPose;
}
